    ...
    const fetchData = async () => {
      try {
        const [gendersRes, coursesRes] = await Promise.all([
          ...
          fetch(`${VITE_BASE_URL_JAVA_BACKEND}/genders`).then((r) => r.json()),
          fetch(`${VITE_BASE_URL_JAVA_BACKEND}/courses`).then((r) => r.json()),
        ]);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4 py-12">
      <Card className="w-full max-w-2xl shadow-2xl">
            ...
            <div className="bg-blue-50 border-l-4 border-fatecride-blue p-4 rounded">
              <div className="flex">
                <div className="flex-shrink-0">
                  <svg
                    className="h-5 w-5 text-fatecride-blue"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                  >
                    <path
                ...
                <div className="ml-3">
                  <p className="text-sm text-blue-700">
                    <strong>Atenção Motorista:</strong> Você poderá cadastrar
                    seus veículos após concluir o registro inicial.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="mt-8 text-center text-sm text-text-secondary border-t border-gray-200 pt-6">
          ...
          <Link
            to="/login"
            className="text-fatecride-blue hover:text-fatecride-blue-dark font-semibold transition-colors"
          >
            Fazer login
          </Link>
        </div>
